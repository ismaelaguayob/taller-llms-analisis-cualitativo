...
```{r}
pacman::p_load(datamedios, dplyr, DT, flextable, gemini.R, ggplot2, jsonlite, purrr, stringr) # Cargamos paquetes necesarios
...
for (i in 1:nrow(noticias_muestra)){
  ...
  for(intento in 1:3){ # Aquí pueden poner más o menos si es necesario
    analisis <- tryCatch(
      gemini(prompt, model = g_model, temperature = g_temperature, seed = g_seed),
      error = function(e) NULL
    )
    if(!is.null(analisis)) break # Si obtenemos el análisis se pasa directo a otra fila
    Sys.sleep(2)
  }
  
  if(!is.null(analisis)) lista_respuestas <- append(lista_respuestas, analisis)
...
Por motivos pedagógicos, vamos a cargar una base de datos más grande previamente analizada.

-   Frecuencia de orientación tecnológica

-   Frecuencia centralidad de la IA

-   Cambio de esta en el tiempo

-   Relación entre medio y orientación tecnológica